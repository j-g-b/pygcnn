...
edge_weights = np.array([0.9])
keep_prob_ph = tf.placeholder(tf.float32)

filter_shape = [[filter_size, n_filter_features, n_node_features], [filter_size, n_filter_features, n_filter_features]]

# Placeholders
picture_x = tf.placeholder("float32", shape=(batch_size, 784))
picture_y = tf.placeholder("float32", shape=(batch_size, 10))

Convolve1 = GraphConvolution(name='first_conv', G=mnist_graph, n_layers=2, filter_shape=filter_shape, partition_resolution=0.1, n_timepoints=n_timepoints, edge_weights=edge_weights, act_fun=tf.nn.elu)
